...
const EditCourse = () => {
  ...
  const fetchCourseData = async () => {
    try {
      ...
      if (data.success) {
        ...
        setChapters(course.courseContent.map(ch => ({ ...ch, collapsed: false })))
  ...
  const handleSubmit = async (e) => {
    try {
      ...
      const token = await getToken()
      const { data } = await axios.post(backendUrl + '/api/educator/update-course', formData,
  ...
  useEffect(() => {
    ...
    if (!quillInitialized.current && editorRef.current) {
    ...
    <div className='h-screen overflow-scroll flex flex-col items-start justify-between md:p-8 md:pb-0 p-4 pt-8 pb-0'>
      <div className='w-full'>
        <div className='flex items-center gap-4 mb-4'>
          <button onClick={() => navigate('/educator/my-courses')} className='text-gray-500 hover:text-gray-700'>
            ← Back
          </button>
          <h2 className='text-lg font-medium'>Edit Course</h2>
        </div>
          ...
          <div className="flex items-center justify-between flex-wrap">
            ...
            <div className="flex md:flex-row flex-col items-center gap-3">
              ...
              <label htmlFor='thumbnailImage' className='flex items-center gap-3'>
                ...
                <img className='max-h-10' src={image ? URL.createObjectURL(image) : existingThumbnail} alt="" />
              </label>
            </div>
          </div>
          <div className="flex flex-col gap-1">
            <p>Discount %</p>
            <input onChange={e => setDiscount(e.target.value)} value={discount} type="number" placeholder='0' min={0} max={100} className='outline-none md:py-2.5 py-2 w-28 px-3 rounded
                    ...
                    {chapter.chapterContent.map((lecture, lectureIndex) => (
                      <div key={lectureIndex} className='flex justify-between items-center mb-2'>
                        <span>{lectureIndex + 1} {lecture.lectureTitle}-{
                          lecture.lectureDuration} mins -<a href={lecture.lectureUrl} target='_blank' className='text-blue-500'>Link</a>-{
