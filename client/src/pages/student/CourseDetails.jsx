  ...
  useEffect(() => {
    if (userData && courseData) {
      ...
      const enrolled = Array.isArray(enrolledCourses) &&
        enrolledCourses.some(courseId => courseId.toString() === courseData._id.toString());
      console.log('Is already enrolled:', enrolled);
  ...
  return courseData ? (
    <>
      <div className='flex md:flex-row flex-col-reverse gap-10 relative items-start
        ...
        h-section-height -z-1 bg-gradient-to-b from-cyan-100/70'></div>
        {/* left columm */}
                ...
                <div key={index} className='border border-gray-300 bg-white mb-2 rounded'>
                  <div className='flex items-center justify-between px-4 py-3 cursor-pointer select-none' onClick={() => toogleSection(index)}>
                    <div className='flex items-center gap-2'>
                      <img
                        className={`transform transition-transform ${openSections[index] ? 'rotate-180' : ''}`} src={assets.down_arrow_icon} alt="arrow icon" />
